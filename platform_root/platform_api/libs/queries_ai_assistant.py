...
SELECT
    ...
    (
        SELECT JSON_LENGTH(h.CONTENTS)
        FROM T_CHAT_HISTORY h
        WHERE h.CONVERSATION_ID = c.CONVERSATION_ID
...
(
    HISTORY_ID, CONVERSATION_ID, HISTORY_SEQ,
    CONTENTS,
    CREATE_TIMESTAMP, CREATE_USER,
    LAST_UPDATE_TIMESTAMP, LAST_UPDATE_USER
)
VALUES (
    %(history_id)s, %(conversation_id)s, %(history_seq)s,
    %(contents)s,
    NOW(), %(user_id)s,
    NOW(), %(user_id)s
)
...
SELECT HISTORY_ID, HISTORY_SEQ, CONTENTS, LAST_UPDATE_TIMESTAMP
FROM T_CHAT_HISTORY
WHERE CONVERSATION_ID = %(conversation_id)s
