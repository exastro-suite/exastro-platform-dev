...
class update_organization_db:
    ...
    def start(self):
        ...
        """
        globals.logger.info('organization database update start (v2.10.0)')
        ...
        with closing(db.connect_platformdb()) as conn:
            with conn.cursor() as cursor:
                ...
                try:
                    ...
                    cursor.execute(queries_db_organizations.SQL_SELECT_ORGANIZATION_DB)
        ...
        for row in organization_db_rows:
            ...
            with closing(db.connect_orgdb(organization_id)) as conn:
                with conn.cursor() as cursor:
                    ...
                    # T_USER_CURRENT_AI_SERVICE テーブル作成（オーガナイゼーション単位）
                    try:
                        globals.logger.info(f"[{self.step_count}/{self.step_max}] - create table T_USER_CURRENT_AI_SERVICE: organization_id:[{organization_id}]")

                        # テーブル存在確認
                        cursor.execute(queries_db_organizations.CHECK_TABLE_EXISTS, ('T_USER_CURRENT_AI_SERVICE',))
                        result = cursor.fetchone()

                        if result['count'] == 0:
                            query = queries_db_organizations.CREATE_TABLE_USER_CURRENT_AI_SERVICE
                            globals.logger.debug(f'EXECUTE SQL:{query}')
                            cursor.execute(query)
                            globals.logger.info(f"[{self.step_count}/{self.step_max}] -- OK: create table T_USER_CURRENT_AI_SERVICE: organization_id:[{organization_id}]")
                            self.ok_count += 1
                        else:
                            globals.logger.info(f"[{self.step_count}/{self.step_max}] -- SKIP: table T_USER_CURRENT_AI_SERVICE already exists: organization_id:[{organization_id}]")
                            self.skip_count += 1

                        conn.commit()

                    except Exception as e:
                        globals.logger.info(f"[{self.step_count}/{self.step_max}] -- NG: create table T_USER_CURRENT_AI_SERVICE: organization_id:[{organization_id}]")
                        globals.logger.error(f"exception:{e.args}")
                        message_id = "500-90035"
                        message = multi_lang.get_text(message_id,
                                                      "organization_db create table T_USER_CURRENT_AI_SERVICE failed. organization_id:[{0}]",
                                                      organization_id)
                        raise common.InternalErrorException(message_id=message_id, message=message)
