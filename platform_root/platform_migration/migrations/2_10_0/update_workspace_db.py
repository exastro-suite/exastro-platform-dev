...
class update_workspace_db:
    ...
    def start(self):
        ...
        """
        globals.logger.info('workspace database update start (v2.10.0)')
    ...
    def __update_workspace_db(self, organization_id, workspace_id):
        ...
        with closing(db.connect_workspacedb(organization_id, workspace_id)) as conn:
            with conn.cursor() as cursor:
                ...
                # 1. T_AI_CONVERSATION テーブル作成
                try:
                    globals.logger.info(f"[{self.step_count}/{self.step_max}] - create table T_AI_CONVERSATION: organization_id:[{organization_id}] workspace_id:[{workspace_id}]")

                    # テーブル存在確認
                    cursor.execute(queries_db_workspace.CHECK_TABLE_EXISTS, ('T_AI_CONVERSATION',))
                    result = cursor.fetchone()

                    if result['count'] == 0:
                        query = queries_db_workspace.CREATE_TABLE_AI_CONVERSATION
                        globals.logger.debug(f'EXECUTE SQL:{query}')
                        cursor.execute(query)
                        globals.logger.info(f"[{self.step_count}/{self.step_max}] -- OK: create table T_AI_CONVERSATION: organization_id:[{organization_id}] workspace_id:[{workspace_id}]")
                        self.ok_count += 1
                    else:
                        globals.logger.info(f"[{self.step_count}/{self.step_max}] -- SKIP: table T_AI_CONVERSATION already exists: organization_id:[{organization_id}] workspace_id:[{workspace_id}]")
                        self.skip_count += 1

                    conn.commit()

                except Exception as e:
                    globals.logger.info(f"[{self.step_count}/{self.step_max}] -- NG: create table T_AI_CONVERSATION: organization_id:[{organization_id}] workspace_id:[{workspace_id}]")
                    globals.logger.error(f"exception:{e.args}")
                    message_id = "500-90032"
                    message = multi_lang.get_text(message_id,
                                                  "workspace_db create table T_AI_CONVERSATION failed. organization_id:[{0}] workspace_id:[{1}]",
                ...
                try:
                    ...
                    # テーブル存在確認
                    cursor.execute(queries_db_workspace.CHECK_TABLE_EXISTS, ('T_CHAT_MESSAGE',))
                    result = cursor.fetchone()
